        ...
        for (row, pressure) in CAMPBELL_PRESSURES.iter().copied().enumerate() {
                ...
                &self.samples[row * self.cells..(row + 1) * self.cells]
            {
                ...
                conductivity[0] += 2.0 * conductivity_residual * conductivity_psi_slope
                    ...
                    / log_conductivity;
                conductivity[1] += -6.0 * conductivity_residual * log_ratio / log_conductivity;
                conductivity[2] += 2.0 * conductivity_residual
...
mod tests {
    ...
    #[test]
    fn campbell_observation_precompute_matches_original_exponent_fma() {
        // Golden generated by pristine Aggregation_SoilParameters ydatc/ydatcks
        // precompute with nontrivial lambda bits that distinguish
        // `-3*lambda - 2` from the original contracted exponent.
        // Driver: /tmp/colm-campbell-observation-audit/campbell_observation_adversarial.f90
        // Flags: gfortran -J/tmp/colm-campbell-observation-audit -O2 -fdefault-real-8 -ffree-line-length-none.
        let problem = CampbellProblem::new(
            CampbellInputs {
                theta_s: &[
                    f64::from_bits(0x3FDAE147AE147AE1),
                    f64::from_bits(0x3FDF5C28F5C28F5C),
                    f64::from_bits(0x3FD7AE147AE147AE),
                ],
                k_s: &[
                    f64::from_bits(0x4020000000000000),
                    f64::from_bits(0x4031000000000000),
                    f64::from_bits(0x4008000000000000),
                ],
                psi_s: &[
                    f64::from_bits(0xC03E000000000000),
                    f64::from_bits(0xC0466D5CFAACD9E8),
                    f64::from_bits(0xC0255D7A03A5A401),
                ],
                lambda: &[
                    f64::from_bits(0x3FD423606C804229),
                    f64::from_bits(0x3FC8AC0585392F7D),
                    f64::from_bits(0x3FE51F4B4E3F9A1C),
                ],
            },
            0.46,
            12.0,
        ...
        // Retention observations (`ydatc`) already match exact source bits.
        assert_eq!(problem.samples[0][0].to_bits(), 0x3FD59CD49C0CB68E);
        assert_eq!(problem.samples[1][0].to_bits(), 0x3FDDA65E3294D3D8);
        assert_eq!(problem.samples[3][0].to_bits(), 0x3FD496DD9746026B);

        // Stored conductivity observations are log10(ydatcks).  The raw
        // original ydatcks bits for these rows are 3FF0A22EED2774D3,
        // 40200F2F1BB2783D, and 3FE5219780A53644 respectively.
        assert_eq!(problem.samples[0][1].to_bits(), 0x3F9144E71065132F);
        assert_eq!(problem.samples[1][1].to_bits(), 0x3FECF34707711A0D);
        assert_eq!(problem.samples[3][1].to_bits(), 0xBFC711A0A2802F4C);
    }

    #[test]
    fn campbell_callback_matches_original_fma_operand_contract() {
        // Golden generated by pristine CoLM202X SW_CB_dist, whose extracted
        // routine hash matches the instrumented copy in
        // /tmp/colm-campbell-callback-audit/source-equivalence.txt.
        // Driver: /tmp/colm-campbell-callback-audit/campbell_callback_original.f90
        // Flags: gfortran -J/tmp/colm-campbell-callback-audit -O2 -fdefault-real-8 -ffree-line-length-none.
        fn bits(hex: &str) -> Vec<u64> {
            hex.split_whitespace()
                .map(|value| u64::from_str_radix(value, 16).expect("valid hex golden"))
                .collect()
        ...
        assert_eq!(
            residual.map(f64::to_bits).as_slice(),
            bits("3FD07B4BDBD9DF14 3FCFEA19D35BD4F3 3FCE45A9841BC7F0 3FCD04651E7685EE 3FCC0221F7310ED6 3FCB2B35C2341E81 3FCA73CD8C71A256 3FC9476BE992C0EC 3FC7652C0EE2052A 3FC6B02D28D55767 3FC36ABFC3253C88 3FC1BADEAD6105C5 3FB7995B31529DBC 3FB123A5DB3F8B23 3FAF7EBF2C8B0E18 3FA1A81711D788E8 3F88F14213DDFCE6").as_slice(),
            "Campbell residual FMA operands must match original callback bits"
        );
        assert_eq!(
            jacobian.map(f64::to_bits).as_slice(),
            bits("3F88AD792143EE6F 3FD259BA4C983055 BF925315218EB8A7 3F88721859AC3D29 3FD73DA236386E47 BF922BD10A2DD797 3F88127FD1249836 3FDEE9AA9B88A65B BF91EBCCF286B452 3F87C727704064BE 3FE265351EFC95F6 BF91B8AF4EED89F2 3F87890A8D264860 3FE4C394225F77B8 BF918E21CACD76B1 3F87543E57BD9F87 3FE6BD64FEE4D659 BF9169AE415EC57F 3F87265AA17ABA64 3FE86DFBE49CF93A BF9149CC6D63705E 3F86D9739D0AA8A6 3FEB3481930DE0EA BF9113F9103D76E4 3F86591C96C378FF 3FEFAEF0E79F559D BF90B91E76766926 3F86274593791DC7 3FF0AF90D8540463 BF909584A76EF2A0 3F8533318DBAC546 3FF49E342F5C56D4 BF8FC9E9B90D2C99 3F84AB6B9EEFF29A 3FF6A9406C982E8A BF8F02C9511566A5 3F8285420BB48A94 3FFDDB573D00C1DA BF8BCEDBFAE4C791 3F81122639B4143D 4000D52F077EB7FA BF899F2C07040622 3F80B7E5D58118A4 40013A4DEF90B4B3 BF8916B372FA2B74 3F7D344EC9F58A8C 400300AACA801396 BF85E2C61CC98C3D 3F7722E65DD7DB04 4003A4A44D89326F BF814DB930C834F6").as_slice(),
            "Campbell Jacobian FMA operands must match original callback bits"
        );

        // Additional pristine SW_CB_dist probe proves the psi/conductivity
        // derivative coefficient `(3*lambda + 2)` is also contracted.
        // Driver: /tmp/colm-campbell-callback-audit/campbell_poscoeff_original.f90.
        let coeff_problem = CampbellProblem::new(
            CampbellInputs {
                theta_s: &[0.42, 0.49, 0.37],
                k_s: &[8.0, 17.0, 3.0],
                psi_s: &[-30.0, -45.0, -12.0],
                lambda: &[0.1, 0.15, 0.45],
            },
            0.46,
            12.0,
        );
        let x = [
            f64::from_bits(0xC030000000000015),
            f64::from_bits(0x3FB000000002899B),
            f64::from_bits(0x40240000000124EF),
        ];
        let mut jacobian = [0.0; 17 * 3];
        assert!(coeff_problem.jacobian(&x, &mut jacobian));
        assert_eq!(jacobian[0].to_bits(), 0x3F304DCEFFA87030);
    }
