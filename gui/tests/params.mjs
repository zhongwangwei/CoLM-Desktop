...
assert.match(params, /language\(\) === 'en' \? 'Tunable parameters' : '可调参数'/);
assert.doesNotMatch(params, /Research-tunable parameters|科研可调参数/);
assert.match(params, /const expert = rows\.filter\(e => !commonField\(e\)\)/);
assert.match(params, /if \(state\.expert && expert\.length\)/);
assert.match(params, /\.filter\(item => state\.expert \|\| item\.visibility !== 'editable-expert'\)/);
...
assert.match(params, /calibration_eligible/);
...
assert.match(params, /set_pft_parameters_batch/);
assert.match(params, /renderPftParameterMatrix/);
...
assert.match(params, /parameterStatusFilter/);
assert.match(params, /item\.aliases/);
assert.match(params, /lct:IGBP:/);
assert.match(params, /lct:USGS:/);
assert.match(params, /reset_field_batch/);
assert.match(params, /reset_process_parameter_field_batch/);
assert.match(params, /if \(result\.changed\) await markResultsStale\(dirs\)/);
assert.match(params, /land_cover_contexts/);
assert.match(params, /EXPERT_SAME_LCT/);
assert.match(params, /MOD_Const_LC\.F90 \/ 当前分类表/);
assert.match(params, /preview_import_parameter_overrides/);
assert.match(params, /apply_import_parameter_overrides/);
...
assert.match(paramsHtml, /id="parameter-search"/);
assert.match(paramsHtml, /id="parameter-status-filter"/);
assert.match(paramsHtml, /id="parameter-export"/);
assert.match(paramsHtml, /id="parameter-import"/);
assert.match(paramsHtml, /id="parameter-import-summary"/);
assert.match(paramsHtml, /专家模式会在各过程分栏显示可调参数和外部过程参数文件/);
assert.doesNotMatch(paramsHtml, /不常用的 case\.nml 参数/);
