...
const ZH_EN = [
  ...
  ['成功候选', 'Successful candidates'],
  ['失败候选比例', 'Failed candidate proportion'],
  ['按候选成员计数，不含基准成员；任一站点执行失败或中断即计为失败，取消和待复核不计为执行失败。有效样本数按站点、变量和时刻单独统计。', 'Counts are per candidate member, excluding the baseline. An execution failure or interruption at any site counts as a failed candidate; cancellation and pending review do not. Effective sample size is calculated separately for each site, variable, and time.'],
  ['失败候选超过 20%；成功样本可能有选择偏差，不能代表完整采样范围。', 'More than 20% of candidates failed. Successful samples may be selection-biased and may not represent the full sampling range.'],
