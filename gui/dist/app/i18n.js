...
const ZH_EN = [
  ...
  ['这是基本设定完成后的可选研究流程。Study 会复制当前算例并独立运行，不修改原算例。', 'This is an optional workflow available after Basic setup. The Study copies and runs the current cases independently without modifying them.'],
  ...
  ['1 · 设计', '1 · Design'],
  ['定义科学问题、方法、站点、分析窗口和样本预算。这里只形成试验方案，不创建目录，也不运行模型。', 'Define the scientific question, method, sites, analysis window, and sample budget. This only forms the experiment design; it creates no directories and runs no model.'],
  ['2 · 创建', '2 · Create'],
  ...
  ['本页定义成员如何生成、多个站点如何组织，以及用哪段输出做统计。尚未运行过基准算例也可以配置，Study 会运行自己的 baseline。', 'This page defines how members are generated, how multiple sites are organized, and which output period is analyzed. You can configure it before an original case has run because the Study runs its own baseline.'],
  ['是什么', 'What it is'],
  ['怎么选', 'How to choose'],
  ['选择后果', 'Consequences'],
  ...
  ['共享模式把同一参数成员应用到全部站点；独立模式为每个站点分别创建一个 Study。', 'Shared mode applies each parameter member to every site; independent mode creates a separate Study for each site.'],
  ['比较同一参数假设在多站点的影响时选共享；站点的基准值、可用参数或输出不同则选独立。', 'Choose shared mode to compare the same parameter assumptions across sites; choose independent mode when sites have different baseline values, available parameters, or outputs.'],
  ['共享模式要求所选参数的 baseline 一致且输出适用于所有站点；独立模式更灵活，但 Study 数量和管理成本随站点数增加。', 'Shared mode requires matching baseline values for selected parameters and outputs applicable to all sites. Independent mode is more flexible, but Study count and management cost grow with the number of sites.'],
  ['分析窗口', 'Analysis window'],
  ['只限定读取和统计结果的连续时段，不会缩短 CoLM 的完整模拟；留空时使用全部可用结果。', 'Limits only the continuous period read and analyzed; it does not shorten the full CoLM simulation. Leaving both dates empty uses all available results.'],
  ['选择与科学问题一致且结果已稳定的时段，避开 spin-up、缺测期或异常强迫期。', 'Choose a stable period aligned with the scientific question, avoiding spin-up, missing-data periods, and abnormal forcing periods.'],
  ['窗口过短会让分位带和参数排序不稳定；包含 spin-up 会混入初始条件影响；更长窗口会增加读取和统计成本。', 'A short window makes quantiles and parameter rankings unstable; including spin-up mixes in initial-condition effects; a longer window increases reading and aggregation costs.'],
  ...
  ['同时派发的任务数，上限为本机检测到的逻辑 CPU。共享模式控制成员/站点任务；运行多个独立 Study 时控制 Study 数且每个 Study 内部并发为 1，只有一个 Study 时则控制其内部任务。', 'The number of tasks dispatched at once, capped by detected logical CPUs. Shared mode controls member/site tasks. With multiple independent Studies it controls concurrent Studies and each Study uses one internal job; with one Study it controls that Study’s internal tasks.'],
  ...
  ['基本设定当前匹配到的 CoLM 内核运行目录；它决定有效物理方案、可采样参数、计划输出和实际数值计算。', 'The CoLM runtime directory currently matched by Basic setup. It determines active physics, sampleable parameters, planned outputs, and the actual numerical computation.'],
  ['这里自动使用当前匹配内核；如需更换，应先回到基本设定修改物理方案并选择对应构建。', 'The currently matched kernel is used automatically. To change it, first return to Basic setup, change the physics configuration, and select the corresponding build.'],
  ['创建 Study 时会记录路径和内核指纹，运行时再次校验；更换内核会破坏结果的可重复性和可比性。', 'Study creation records the path and kernel fingerprint, which are checked again at runtime. Changing kernels breaks result reproducibility and comparability.'],
  ['先明确要评估哪些参数假设会让哪些输出发生多大变化，再选择方法、站点方式、分析窗口、样本数、随机种子、并发和内核。', 'First define which parameter assumptions may change which outputs and by how much, then choose the method, site mode, analysis window, sample count, random seed, concurrency, and kernel.'],
  ...
  ['选择 OAT 或 LHS、站点组织方式、分析时段、样本数和并发数。', 'Choose OAT or LHS, site organization, the analysis period, sample count, and concurrency.'],
  ['这些设置决定如何覆盖参数空间、要计算多少成员，以及结果对应哪段模拟时段。', 'These settings determine how the parameter space is covered, how many members run, and which simulation period the results represent.'],
