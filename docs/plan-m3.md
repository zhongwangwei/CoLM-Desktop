...
用 `oracle` 的判官比对新旧站点文件，把差异逐条写进 `PROVENANCE.md`：
预期是 `soil_texture` 4→8，四个反照率不变（CN-Cng 恰好是 isc=10），
`lakedepth` 1.0→0.0，`elevation` 138.0→144.144…，`elvstd` 0.0→0.496…，
`sloperatio` 0.0→0.003576。**实际差异与预期不符时停下来查清楚，不要继续。**
...
对比 Step 1 记下的指标。**必须解释的**：`mkinidata` 日志里的
`BVIC [-] is in (0.10, 0.10)`（旧的是 0.23）；湿季窗口 `f_rnof` 的变化。
