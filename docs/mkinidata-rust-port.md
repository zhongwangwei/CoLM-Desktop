...
standard case paths, LULCC year, start timestamp, LAI year, and enabled cold-start controls,
then scans the selected landpatch year for every block; --block restricts that scan, and LCT
requires an explicit igbp/usgs classification option. Spatial urban, observed spatial state, PC
