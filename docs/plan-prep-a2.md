...
| 1 | 手填高度进产物 | ✅ `b4140d6` |
| 2a | 抽出参数解析 | ✅ `c3c79bf` |
| 2b | `colm-cli` 两个子命令 | ✅ 同上 |
| 2c | GUI 走 sidecar | ✅ `03e64df` |
| 3 | 子栏界面 | ✅ `d05a942` |
| 3b | 真机验收 | ✅ 查出三个真问题 |
| 4 | 端到端 FI-Kumpula | ✅ `c0764c0` |
| 5 | `colm-cli new --met` | ✅ `20e3bd1` |
| 6 | 界面上也能指定强迫场 | ✅ `c03cc5f` |

**端到端判据**（`oracle/tests/forcing_prep.rs`，`#[ignore]`）：

```
f_tref        264 个值，255.71 – 273.90 K
HEIGHT_T      48.05（不是 NaN）
Precip        源 Rainf+Snowf 5.06786063762179 == 产物 5.06786063762179
```

后两条才是判据的价值所在 —— 只验「跑通了」的话，用原始强迫场也跑得通。

### 期间修掉的、不在原计划里的

| | 什么 | 怎么发现的 |
|---|---|---|
| `5d42291` | 额外变量与主变量撞名 → 降水翻倍 | 写界面的 agent 标出风险，我验证成立 |
| `796fa2f` | 产物目录要给建议值 | 验收走不到转换那步逼出来的 |
| `c65b1ee` | **产物目录不存在，没人建** | 真机验收 —— 单测一直往 `/tmp` 写，那儿永远存在 |
| `c68633a` | 后端对「加两次」毫无防备 | 验收 agent 拿修复前的 payload 直接喂后端 |
| `d892622` | `#fsrc` 选完不被记住 | 验收 agent 实测推翻了我给的绕法 |

### 三个反复出现的形状

**① 「做了动作，但依赖它的那一半没跑」** —— 出现过四次：
`#kernel` 赋值不触发 `onchange`、`restoreRecent` 不派发 `change`、
`wirePickers()` 一次性绑定管不到动态 DOM、`REMEMBERED` 表漏字段。
四次都**不报错**。

**② 「同一段代码抄几遍，改的时候不知道在改哪一份」** —— 出现过两次，
第二次是在读着第一次留下的注释的情况下又犯的
（`copy_attributes` 与 `ensure_parent`）。

**③ 「约定失败的方式是静默回落到一个看起来合理的答案」** ——
`sibling()` 推出原始强迫场，两层各犯一次（CLI 与 GUI）。
而**观测那条路从一开始就是对的**：用户填的优先、约定回落。
正确的问法不是「这里缺什么」，是「同样的事情别处是怎么做的」。
...
**与 Task 3 分开是有意的。** 上一轮 Task 15 的验收单独烧了 190k tokens
和 197 次工具调用 —— 写代码和点界面混在一个任务里，两头都做不好。

### 验收工具的能力与限制（已核实）

脚本在 scratchpad（`ax_card.sh` / `clickrev.sh` / `setpopup.sh` …）。

| 要做的事 | 怎么做 |
|---|---|
| 读某一块 | `ax_card.sh "标题" [深度]` —— 只遍历那一块 |
| 点按钮/勾选 | `clickrev.sh`（**反向 DFS**，长表格在前时倒着走才点得到） |
| 选下拉 | `setpopup.sh "当前值" "要选的项"` —— 走 AX 点击 |
| 读按钮是否禁用 | `btnstate.sh` / `readbtn.sh` |
| **填文本框** | ⚠️ 见下 |

**填文本框是这次的难点。** `settext.sh` 用的是 `keystroke`，
而**沙箱合成不了键盘输入**（`setpopup.sh` 的注释就是因为这个才写的）。

已知的绕法：改
`~/Library/Application Support/edu.sysu.colm.desktop/recent.json`，
`restoreRecent()` 启动时会恢复 —— 但那只覆盖**被 `wirePickers` 记住的
字段**（有 `data-for` 的那些），三个高度输入框不在其中。

所以验收分两级：

**必须验到的**（不需要填文本框）：

1. 选一个 Urban-PLUMBER 文件 → 探一探 → **8 行槽位表画出来**，
   第 5 槽是 `Wind_E` 而不是空
2. 第 4 槽能加 `also_add`（下拉里有 `Snowf`）
3. 三个高度框**是空的**，且旁边说明了不填的后果
4. 转换按钮**默认禁用**；点「这些映射我看过了」之后启用
5. **改一行映射之后按钮重新禁用** —— 这条最容易漏，
   而漏了比不做确认更糟（界面上显示着「已确认」）

**尽力验的**（要填文本框）：

6. 填三个高度 → 转换 → 产物路径显示出来

第 6 条若因为键盘合成卡住，**不要硬试**。记录为限制，
用 CLI 验证同一条路（`colm-cli forcing-convert --height 48.05,48.05,48.05`
已经实测过）。**界面能不能填**与**填了之后对不对**是两件事，
后者已经有覆盖。

### 三个已知坑（来自上一轮）

1. 点站点行的文字触发不了 `onclick`，要点那一行的 `AXCheckBox`
2. 分开的多次调用会读到滞后快照 —— 点击后 `sleep 1` 再读
3. **参数页有 737 个字段，AX 树大到会让查询超时**（`clickfast.sh` 报 `-1700`）。
   绕法是**不从界面读，直接读磁盘产物** —— 验收要的事实在磁盘上，
   不在 AX 树里

### 改了 `gui/dist/` 必须重新编译

Tauri 的前端资源是**编译期嵌入**的：

```bash
cd gui/src-tauri && cargo build && ./target/debug/colm-desktop-gui > /tmp/gui.log 2>&1 &
```
