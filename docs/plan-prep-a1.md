...
| 1 | 恒等转换（管道地基） | ✅ `f60658b` |
| 2 | 单位换算 | ✅ `72a9d11` |
| 3 | 槽位映射接受用户指定 | ✅ `182b02d` |
| 4 | 转换管道 | ✅ `b788642` |
| 4b | 多源合成并保留相态 | ✅ `e1064d2` |
| 5 | 命令行入口 | ✅ `20936a4` |
| 6 | CN-Cng 端到端对照 | ✅ `c4be924` + `191fea7` |

**A1 完成。** 端到端判据：`identical: 129 variables, 10 dimensions
(ignoring ["create_time"])`。

**期间修掉的、不在原计划里的：**

- `3e8a4fa` 降水的规范单位是 `kg/m2/s` 不是 `mm/s`（计划写错，已进过代码）
- `3f26371` 属性搬运丢 `_FillValue`（三处抄了同一段代码，三处都丢）
- `62727d1` 内核测试的临时目录跨进程撞名（两个 agent 各自报过 flaky）
- `f297bf4` `--start` 也要校验（原先只拦 `--end`，同一个理由漏了一半）
- `191fea7` **槽位没消费的变量也要搬** —— Task 6 撞出来的真 bug：
  `reference_height_v/t/q` 不搬，`summarize` 静默回落成 `NaN`，
  写进 namelist 后 CoLMDEBUG 内核 RangeCheck 直接 `SIGILL`，
  而报出来的是「内核编进了 CoLMDEBUG」

**Task 6 的做法值得记：** 它文件层面那条测试已经绿了（0.83 秒），
本可以交差。它跑了慢的那条，撞上 `SIGILL`，一路追到根因，验证那是
唯一的缺口，然后**把验证用的补丁撤掉**，提交了一个当前会失败的
`#[ignore]` 测试 —— 因为改 `convert.rs` 超出了它的范围。

**一个诚实失败的测试，比一个放松了判据的绿测试有价值得多。**

**顺带查证的三条「已知问题」，只有一条半是真的：**

| 记录里写的 | 查证结果 |
|---|---|
| `colm-cli new` 只校验 `--end` 不校验 `--start` | ✅ 真的，已修 `f297bf4` |
| 默认时间窗口比强迫场早一天 | ❌ **不成立**。90 个站逐一比对，窗口与覆盖范围零不一致 |
| `colm-kernel` 少数测试对时序敏感 | ⚠️ 现象真，**根因记错了**。不是时序，是固定的 `/tmp` 目录名跨进程撞车，`62727d1` |

**问题记录会过期，也会从一开始就记错。** 「对时序敏感」听起来像是
诊断，其实只是把「重跑就好」换了个说法 —— 而真正的原因（两个 cargo
进程抢同一个目录）指向一个五分钟就能修的东西。带着一条错的诊断，
比没有诊断更耽误事。
...
| 处 | 计划里写的 | 实测 |
|---|---|---|
| Task 2 | `assert_eq!(…233.15)` | 数学上不可满足，差 1 ULP |
| Task 4b | 降水单位 `mm/s` | `kg/m2/s`（111 个文件全是） |
| Task 5 | `.and_then(\|v\| v.attribute(..))` | 过不了借用检查 |
| Task 5 | `SlotPlan` 三个字段 | 少了 `also_add` |
| Task 5 | `--slot N=名字:单位` | 表达不出降水合成 |

**教训**：规格里的代码片段是**照着记忆写的**，字段名、API 签名、
测试字面量都可能错。执行时以代码和实测为准，别改实现去迁就规格。
