...
**冬季三行全部精确一致。** 湿季三行的差异只在末位（RMSE 13.4 vs 13.5、
R² 0.455 vs 0.456），是舍入，不是算法分歧 —— 所以下面 Task 7 的验收对
n / R² 用精确相等，对 RMSE / bias / KGE 用容差。
...
fn check(hist: &str, spinup: usize, rows: &[Row]) {
    ...
    for r in rows {
        ...
        assert!((m.r2 - r.r2).abs() < 5e-4, "{} R² {} vs {}", r.obs, m.r2, r.r2);
