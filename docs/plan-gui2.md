...
### 任务 2：站点扫描（后端 + 前端，**已完成**；批量队列并入任务 4）
...
1. **`Site` 的字段比计划的多。** 计划写的是
   `name / site_file / has_forcing / has_observation / urban`，实现里直接
   镜像 `colm-cli scan` 的 `SiteInfo`（多了经纬度、地类、时间范围、
   `problem`）。理由：那些数据 CLI 已经读出来了，再定义一个更窄的结构体
   只会让界面为了显示经纬度再跑一次扫描。
2. **两边的结构体靠一条测试拴住**，不是靠约定。`colm-cli` 在引擎 workspace、
   GUI 在另一个，两者不互相依赖，所以 `Site` 与 `SiteInfo` 各写各的。
   `it_parses_what_the_real_cli_prints` 跑**真 CLI**、解析**真输出**、
   断言 90 个站点 —— 那边改了字段名，这条先红。
   （验过它不是静默跳过：不给 `PLUMBER2_ROOT` 是 0.00 秒，给了是 0.08 秒；
   把 90 改成 91 立刻红。）
3. **批量队列挪去任务 4。** 它要改事件模型，和那边是同一件事，分开做会
   把 `run://` 三个事件改两遍。

前端这一屏把「有多少不能跑 / 不能评估」直接写在标题下，而不是让人自己数
一列图标：`10 个站点 · 2 个城市 · 1 个无观测 · 1 个读不了`。
四条分支都在 Chromium 里走过（普通站显示经纬度、城市标「城市」、
读不了的标红且 tooltip 是错误原文、无观测的标出来），选中会把路径填进
新建向导 —— 那是它唯一的去处，不必再让人复制粘贴。
...
所以**不需要进度输出**，界面直接同步调即可。冷缓存第一次是 1.35 s ——
量的时候先跑一遍热身，否则会把页缓存的代价算进解析里（我第一次量出
「快扫比全扫还慢」，就是这么来的）。

多了两个计划里没写的东西：

- `--quick` 开关。全扫要打开 90 个强迫场文件（最大 15 MB），
  而界面第一屏其实只要经纬度与地类。0.07 vs 0.35 秒的差距不算大，
  但网络盘或机械盘上会拉开。
- `problem` 字段。**一个坏文件不该毁掉整次扫描** —— 90 个里有一个读不了，
  其余 89 个仍要列出来，坏的那个把错误原文带给界面。
  顺带一条：读不出地类时**不判成城市**，否则一个坏文件会被送进 URBAN 路径。

- [x] **步骤 3：验收**

```bash
cargo test -p oracle --test scan     # 两条，没数据时跳过
```

两条测试分别钉住两套命名约定：PLUMBER2 90 个站点全部配到强迫场与观测、
一个都不判成城市、`landtype` 全有值；Urban-PLUMBER 全部配到强迫场、
全部判为城市、`landtype` 全为空。**这两条是对称的**，漏了任一套的话，
那一套的全部站点都会在界面上被报成「不能跑」。
...
### 任务 5：控件类型、按内核过滤、九分类与专家模式（**已完成**）

**文件：** 新建 `gui/dist/app/params.js`

§1.5c 的三条（控件类型、枚举下拉、按内核过滤）与 §1.2 的九分类、
普通/专家模式都做完了。实测（AU-Preston 的城市算例，`nl_colm` 页签）：

| | 分节 | 行数 |
|---|---|---|
| 普通模式 | 站点 8 / 时间 7 / 路径 3 / 物理开关 1 / 输出 2 / 其他 1 | 22 |
| 专家模式 | 站点 21 / 时间 15 / 路径 4 / 城市 4 / 土壤 6 / 物理开关 27 / 输出 11 / 其他 55 | 143 |
| 过滤 `spinup` | 时间 5 / 物理开关 1 | 6 |

**「其他」有 55 个**。这不是缺陷，是计划里预期的信号 —— 兜底分类堆得越高，
越是在提醒该给它们归类。留着这个数，别急着为了好看去拆。

**`expert` 与 `showIrrelevant` 是两个开关，不是一个。** 前者问「这份配置设没设过」，
后者问「当前内核编没编进去」。压成一个的话，想看全部已设字段的人会被迫连
68 个用不上的一起看 —— 与 §1.7 里两个 spin-up 同名同一个道理。

分节而不是分页签：页签仍按 namelist 组分，那决定**写进哪个文件**，是另一回事；
`nl_colm` 有 214 个字段，只有它需要再分（另两组是 35 与 482，后者另有专页）。

`xtask/tests/params_groups.rs` 两条守着：九个分类都在、**`other` 必须在最后**
（顺序即优先级，它的 match 恒为真，排在谁前面就把谁整个吃掉 —— 反证过会红），
以及白名单里的名字都是真字段（写错一个，那个字段在普通模式下永远不出现，
而「少显示了一个字段」是没人会报的故障）。

两张表都生成了，长在 `colm-schema` 的 `Field` 上（`values` 与 `requires`），
`describe_fields` 顺带就传给了前端 —— 不必新增命令。

**扫描范围是两个不同的问题**，这点是做的时候才发现的：
取值必须**包含** `MOD_Namelist.F90`（几个枚举的规范取值只写在那里的校验分支
里，连大小写归一一起做），守护必须**排除**它（每个字段都在那里声明并出现在
`namelist` 语句里，一并扫的话答案永远是「到处都用，无守护」）。
一开始两边都排除，结果 TRACER 的三个枚举整个消失。

还踩到两处，都记在代码注释里：

- **大小写别名不是不同的选项。** `CASE ('CAPPA2003', 'Cappa2003', 'cappa2003')`
  是校验器接受的三种写法，同一个选择。不合并的话下拉框给 6 项而实际只有 2 种。
- **只有一个取值的不是枚举**，是一次哨兵比较（`trim(DEF_file_GIEMS) == 'null'`）。
  给一个只有一项的下拉框比文本框更糟：它宣称自己知道全部合法取值，其实不知道。

最终 12 个枚举字段、68 个带宏依赖的字段。实测 `waterheat` 内核下 68 个用不上、
`urban` 下 63 个 —— 差的正是 URBAN_MODEL 那 5 个。

`DEF_URBAN_type_scheme` 的人工条目由 `crates/colm-schema/tests/curated.rs`
守着：它去读 `MKSRFDATA.F90` 里 `CALL landurban_build` 那一行，
确认上方最近的 `#ifdef` 仍是 `URBAN_MODEL`。反证过它会红。

**`check-gui` 也补了一条**：校验 `import { X } from './y.js'` 里的 X
在 y.js 里确实 `export` 了。写这一步时踩到 —— 一次编辑把 `renderFields`
的 `export` 吃掉，浏览器只报一句「does not provide an export named」、
页面整个不动，而 `check-gui` 全绿。反证过新检查会抓住它。
（第一版还报了 5 条假警报：JS 标识符允许 `$`，而我的字符集漏了它。）
...
### 任务 6：482 个输出变量的独立页面（**已完成**）

**文件：** 新建 `gui/dist/app/histvars.js`

- [x] **步骤 1：搜索 + 开关列表**

**实测渲染 482 行只要 16 毫秒**（macOS ARM / Chromium，直接量
`renderHistVars` 那一次 DOM 构建）。计划说低于 100 ms 就不引入虚拟滚动 ——
那就不引入。

量的时候踩了一次：第一版把 `await wait(800)` 算进了耗时，得到 803 ms。
**等待不是耗时** —— 要量的那段代码之外的东西一律不能进计时窗口。

- [x] **步骤 2：接上闸门表**

比计划预想的更有内容：闸门表有 **456** 条，而开关有 **482** 个，
交集只有 421。**对不上的 61 个（多为 `DA_*`）如实报「未知」**，
不当成能写 —— 当成能写会让人以为勾上就有输出，而那正是这一页要防的事。

条件分两层，闸门表两层都记着：编译期的宏（由内核 `manifest.json` 回答）
与运行时的开关（由这份算例配置回答）。运行时条件是**原文**，只求
`DEF_X` 与 `.not.DEF_X` 两种形状；别的形状返回「需要人工判断」并把原文给人看
——**给一个可能反了的结论比不给更糟**。
...
命令叫 `hist_vars(text, kernelDir)`，返回全部 482 条而不只是可写的那些
—— 界面要显示的是「这一个为什么写不出来」，只回可写的那批就答不出。

- [x] **步骤 3：验收 + 提交**

---

### 任务 7：结果页 —— 指标表 + 观测对比图（**已完成**）

**文件：** 新建 `gui/dist/app/results.js`

`colm-cli metrics` 加了 `--json`。**没有去解析那张文本表** —— 解析格式化输出
正是这个仓库一直避免的脆弱做法，而且 `--json` 顺带能把配对点一起带出来。

**一次拿全**：指标表、双线图、散点图用的是同一批配对结果。分三次跑等于把同一份
NetCDF 读三遍，而且三者可能因参数不一致而对不上。为此给 `colm-hist` 加了
`pair_with_time`，**并让原来的 `pair` 委托给它** —— 那条聚合规则
（至少一个半小时 `qc==0`，取好的那些的平均）是实测定下来的，
两份实现迟早分叉，而分叉的表现是「图上的点跟指标对不上」。

JSON 与文本表逐项相同，且 `点数 == n`（说明两条路没分叉）：

```
Rnet   n= 256 rmse= 14.71 r2=0.986 kge=+0.829
Qh     n= 253 rmse= 46.14 r2=0.530 kge=-11.558  KGE 不可信：观测均值 2.8 接近零
Qle    n= 254 rmse= 32.21 r2=0.044 kge=-1.417
```

正是 design.md §2.8 的参考值（Rnet R²=0.986、Qh n=253、Qle n=254）。

观测文件按 §1.4 自动填：从站点库里按算例名找 `obs_file`。找不到就留空，
由输入框兜底 —— 命名不合约定的数据仍然要能用。
...
- [x] **步骤 3：散点图 + 1:1 线**，用于一眼看偏差结构。

- [x] **步骤 4：批量汇总**

**前端循环，没有新开后端接口。** 实测一次评估热缓存 0.03 秒
（冷缓存首次 1.39 秒），90 个站点串行约 3 秒 —— 为此加一个批量命令，
省下的时间还不如它带来的第二份参数处理逻辑值钱。照样报进度，
因为冷缓存那一次是要等的。

默认只汇总一个变量，其余靠按钮切换：**一张 90 行 × 5 变量的表读不出任何东西**。
表头可点排序 —— 那是这张表唯一的用处，90 行里找最差的几个靠眼睛扫不出来。

**没有观测的算例要记名。** 一张少了几行的表，不说明少了谁的话，
看的人会以为那几个站的评估结果是零。实测：
「2/4 个算例有结果；2 个没有：AU-Preston（没有观测文件）…」
...
**身份字段按前缀挡，不逐个列名字。** `SITE_*`、`DEF_dir*`、`DEF_CASE_NAME`、
`DEF_forcing_namelist` 不进预设 —— 它们是**算例身份**不是参数，混进去会让
套用变成「悄悄换了个站点」，而算例名与目录名都不变，从外面看不出来。
按前缀的好处是上游加一个新的 `SITE_` 字段时自动也被挡住；逐个列的话，
**新字段会默认进预设**，而那是最糟的方向。

**被挡下的要说出来。** `save_preset` 返回被跳过的字段名，界面照实显示
（实测那份城市算例：「9 个身份字段未收进去（DEF_CASE_NAME、SITE_fsitedata…）」）。
静默丢弃会让人以为存进去了，然后在套用时才发现站点没跟着过来。

**一个字段设不上就整份放弃。** 部分套用会留下一个既不是原状也不是预设的状态，
而且用户不知道套进去了哪几个。（写这条时发现注释与代码矛盾 ——
注释写着「其余照套」而代码是 `return Err`。代码是对的，改了注释。）
...
### 任务 10：两个 spin-up 都进界面（**已完成**）

见 §1.7。**先在 UI 文案上把它们分开命名**：模型侧叫「预热（spin-up）」，
评估侧叫「丢弃前 N 条记录」。**不要**在两处都写「spinup」。

解析那一半在任务 4 就做了（payload 只改一遍），评估侧的入口在任务 7 做了。
这里补的是模型侧的说明，以及**把两个名字彻底分开**。

界面上的措辞纪律：模型侧一律叫**预热**，评估侧一律叫**丢弃前 N 条**，
两处的提示各自指认对方（「与『时间』分类里的预热不是一回事 —— 那个的单位
是轮数」）。五个 `spinup_*` 字段都带 ⓘ 标记 —— **一个只在悬停时才出现的提示
等于没有**，得先让人看出这里有说明。

`spinup_repeat` 的说明里写明了 `MOD_Hist.F90:235` 在 `itstamp <= ptstamp`
时直接 RETURN，所以预热期不写 history、不会被算进指标。不说这句的话，
用户会以为预热期的输出污染了结果 —— 而那正是两者最容易混起来的一点。

`xtask/tests/spinup_naming.rs` 两条守着。写这条测试本身踩了两次，都是
**「好行为被判成违规」**：

1. 第一版按字节窗口取「输入框附近的文本」，把紧随其后那段**故意提到预热**
   的说明圈了进来 —— 而那段说明正是应该有的。
2. 改成只看 `<label>` 之后仍然红：`title` 属性里也在指认另一个名字，
   同样是应该的。

最终只查**可见文本**（开标签之后的部分）。教训：一条静态检查如果会因为
「把话说得更清楚」而变红，那是检查错了，不是代码错了。

- [x] **步骤 1：`parse_progress` 认 spin-up 格式（写失败的测试先）**
...
三档断点，用**命名区域**而不是列数 —— 折叠时是「结果区换个位置」，
不是「三列挤成一列」。实测三个宽度：

| 视口 | 布局 | 横向滚动 | 重叠 | 图表宽度 |
|---|---|---|---|---|
| 1600 | `230px 935px 420px` 三栏 | 无 | 无 | 390 |
| 1200 | `230px 955px`，结果区移到主区下方 | 无 | 无 | **897** |
| 760 | 单栏，三块依次排下 | 无 | 无 | **687** |

760 那一档还查了「有没有任何元素的右边界超出视口」—— 空。那是横向滚动的
源头，只看 `scrollWidth` 有时会被 `overflow:hidden` 掩盖。

**图表得真的跟着变。** uPlot 的宽度是建图那一刻算死的，容器变窄它不会缩。
CSS 那边 `overflow:hidden` 只是不让它撑页，**裁掉一半的图仍然是坏的** ——
所以加了一个 resize 监听（120 ms 防抖，拖窗口时每秒能触发几十次）
并在图被移出文档时清理注册表。

顺带修了三处会撑破布局的：`input` 的 `min-width: 0`（flex 子项默认
`auto`，一个长路径就能把整行顶出容器）、`pre` 与首列的
`overflow-wrap: anywhere`（日志里全是长路径）。

`xtask/tests/style_layout.rs` 两条守着：三档断点都在，以及
**深色块里只许出现变量声明**（反证过：塞一条 `.warn { color: … }` 会红）。
深色重复写一遍规则是最容易分叉的地方 —— 改了浅色忘了深色，
而写代码的人十有八九只看着一种模式。
