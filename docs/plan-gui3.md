...
**✅ 已完成**（`7f626d2`，2026-08-20）。六步验收全绿，⑥ 画出图，
全程没有手动改过路径。
...
1. **只需要换算例根目录。** 实测把 `Sitedata`/`Forcing`/`Observation`
   留在 `/tmp/has space/` 下、`--out` 指向无空格目录，三段全部 ok ——
   那三样只被 netCDF 按路径打开读取，不经过 `mkdir -p`。所以示例数据
   仍留在 `app_data_dir()` 下，只有算例根目录改到了 `~/CoLM-cases`。

2. **不用 `~/Documents`。** 那是 macOS 的 TCC 保护目录，未公证的开发版
   直接访问会弹系统权限对话框 —— 同样打断「装完就能跑」，只是换了个
   打断方式。

3. **`restoreRecent` 不派发 `change`**，所以恢复出来的旧路径不会自动
   被标红，要在 `boot()` 里补一次显式调用。这与之前 `#kernel` 那个
   「选了内核但没触发 onchange」是同一类问题 —— **凡是「赋值 + 依赖
   事件做后续」的地方都要检查一遍**。

### 验收工具的第四个坑（新发现）

已知的三个（点站点行要点 `AXCheckBox`、分开调用会读到滞后快照、
沙箱无法合成键盘输入）之外，再加一条：

**参数页有 737 个字段，AX 树大到会让查询超时。** `clickrev.sh` 在那页
偶发单次卡数分钟，`clickfast.sh` 的单发 whose 查询会报 `-1700`。
绕法是**不从界面读，直接读磁盘产物**（history 目录、`colm-cli series`）
—— 验收要的是「这一步做成了没有」，而那个事实在磁盘上，不在 AX 树里。
