...
**但仍然没有人用鼠标真正用过它。** 全部前端验收都是在 Chromium 里对着
**真后端导出的载荷**做的（做法见下），真窗口只验到「页面加载并接上后端」。
里程碑 8 的验收词是「双击可跑并出图」，其中「点下去跑一次真模型」那一半
仍然只有命令行走过。

### GUI 能做什么

**进门先分流，然后走六步。** 启动时先问「这次要跑什么」——「站点」「区域」
「全球」三档。三种域要的前处理、地表数据与并行设置都不一样，将来各自展开
自己的步骤链；**现在只有「站点」能点，另外两档是灰的（「暂不支持」）**，
而不是点了报错——一个能点但必然失败的入口比一个灰着的更糟。这道门每次
启动都弹，它是分流点而不是一次性欢迎页。

选了「站点」之后是六步，**顺序由依赖链定，不是按界面好看排的**：
...
| ④ | 参数 | 要先建过算例；时间与预热 · 向导之外的 namelist 字段 |
| ⑤ | 运行 | 同上 |
| ⑥ | 结果 | 同上 |
...
两个窗口仍是 `identical: 129 variables, 10 dimensions`。
...
谁也看不出它们是一条流水线** —— 现在左栏那六步就是流水线本身，每一页
...
栅格门槛没了，但**从 GUI 一路点下来跑城市示例，目前还跑不通**，卡在两个与
rawdata 无关的地方。两条都是实测复现的，记在这里而不是等下一个人再撞一次：
...
1. **算例目录里不能有空格。** CoLM 建目录用的是不加引号的
   ...
   算例目录之后，CN-Cng 六步全程跑通（9 个月度 history、曲线 8760 点）。
2. **默认时间窗口比强迫场早了一整天。** 不给 `--start` / `--end` 时，
   AU-Preston 推出来的窗口是 `1992-12-31`，而强迫场第一条记录在
   ...
   当天的时刻。** CLI 与 GUI 同样复现；上面那条 `identical: 146 variables`
   用的是显式的 `--start 1993-01-01`，绕开了它。
