...
内核编排层、两张从 CoLM 源码生成的表（配置字段与输出变量），以及模型-观测
指标层。还没有 GUI。
...
CI 分两层。每个 PR 在 ubuntu / macOS / Windows 三平台跑 **166/185** 条测试 ——
`cargo test --workspace --lib --bins` 的纯计算部分（145 条），加上五条各自点名的
集成测试：判官、namelist 往返、schema 漂移、输出变量闸门表的经验校验与其漂移。
这些只需要源码与已入库的黄金文件。其余 19 条要 5.5 GB 的 PLUMBER2 与 38 GB 的
rawdata，只能在带那些数据的自托管 runner 上跑；「它们没跑」这件事会在 PR 界面上
以警告形式出现，而不是静默缺席。
